...
/*
 * Default keymap for the Epomaker RT82.
 *
 * USB wired keyboard with VIA support.
 */
...
/*
 * Fn + Backspace        = next RGB pattern  (RM_NEXT)
 * Fn + RShift + Bksp    = previous RGB pattern (RM_PREV)
 */
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    if (keycode == RM_NEXT && record->event.pressed) {
...
const uint16_t PROGMEM encoder_map[][NUM_ENCODERS][2] = {
  ...
  [3] = { ENCODER_CCW_CW(KC_AUDIO_VOL_DOWN, KC_AUDIO_VOL_UP) }
};
#endif
